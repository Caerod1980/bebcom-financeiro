...
const Header = ({ onMenuClick, isMobile, sidebarOpen }) => {
  ...
  return (
    <header
      className={`
        bg-white border-b border-gray-200 px-4 py-2 print:hidden
        ${isMobile ? 'fixed top-0 left-0 right-0 z-10' : 'relative w-full'}
      `}
    >
              ...
              <div className="absolute right-0 mt-2 w-48 bg-white rounded-lg shadow-lg border z-30">
                <div className="p-3 border-b">
                  <p className="text-sm font-medium text-gray-900">
                    {user.name || 'Rodrigo Caetano'}
                  </p>
                  <p className="text-xs text-gray-500">
                    {user.email || 'caerod@gmail.com'}
                  </p>
                </div>
